...
module.exports = function(schema, options, version) {
  ...
  let generateType;
  if (options.withDiscriminators && typeof options.type === 'function') {
  ...
  function esOptions() {
    ...
    if (options.withDiscriminators) {
      if (generateType) {
        options.type = generateType(kind);
      } else {
        options.type = utils.lcFirst(kind); // override each time, cause it suppose to have several schemas
      }
    ...
    if (!options.mapping || options.withDiscriminators) {
